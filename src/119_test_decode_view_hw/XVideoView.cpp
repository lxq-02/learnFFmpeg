...
bool XVideoView::DrawFrame(AVFrame* frame)
{
    ...
    case AV_PIX_FMT_NV12:
        ...
        {
            memcpy(_cache, frame->data[0], frame->linesize[0] * frame->height); // Y
            memcpy(_cache + frame->linesize[0] * frame->height, frame->data[1], frame->linesize[1] * frame->height / 2); // UV
        }
        else // 逐行复制
        {
            for (int i = 0; i < frame->height; i++)
            {
				memcpy(_cache + i * frame->width, frame->data[0] + i * frame->linesize[0], frame->width); // Y
            }
            for (int i = 0; i < frame->height / 2; i++) // UV
            {
                auto p = _cache + frame->height * frame->width; // 移位Y
				memcpy(p + i * frame->width, frame->data[1] + i * frame->linesize[1], frame->width); 
            }
        }

        // frame->data[0] + frame->data[1]
        return Draw(_cache, frame->linesize[0]);
